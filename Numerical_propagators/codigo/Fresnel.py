...
fase1=np.exp2((1j * mt.pi / (lamb * z)) * (np.power(n * dx, 2) + np.power(m * dy, 2)))
fase1=U*fase1

mostrar(np.abs(fase1),"fase1")
# Calculo de la transformada de Fourier

campo = np.fft.fftshift(fase1)
mostrar(np.abs(campo),"fft shift 1")
ft=sp.fft.fft2(campo)
mostrar(np.abs(ft),"fft")
ft = np.fft.fftshift(ft)



fase2=np.exp2(1j*2*mt.pi*z/lamb)/(1j*lamb*z)
fase3=np.exp2((1j * mt.pi / (lamb * z)) * (np.power(n * dx_, 2) + np.power(m * dy_, 2)))
