...
fase1=dx*dy*np.exp2(1j*mt.pi/(lamb*z)*((dx*(dx-dx_)*(np.power(n,2)))+(dy*(dy-dy_)*(np.power(m,2)))))
...
fase2=np.exp2(1j*mt.pi/(lamb*z)*((dx*dx_*(np.power(n,2)))+(dy*dy_*(np.power(m,2)))))
...
fase3=np.exp2(-1j*mt.pi*((dx_*(dx-dx_)*(np.power(n,2)))+(dy_*(dy-dy_)*(np.power(m,2)))/(lamb*z)))
