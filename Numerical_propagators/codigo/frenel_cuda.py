...
__global__ void Fresnel(float *U_gpu,cuComplex *dest_copia, cuComplex *dest_gpu, float *m_gpu, float *n_gpu, float dx, float dy, float lamb, float z, float pi, int N, int M)
{
    ...
    dest_copia[i2].x=cos(pi*(pow(n_gpu[i2]*dx,2)+pow(m_gpu[i2]*dy,2))/(lamb*z));
    dest_copia[i2].y=sin(pi*(pow(n_gpu[i2]*dx,2)+pow(m_gpu[i2]*dy,2))/(lamb*z));

	dest_gpu[i2].x = U_gpu[i2]*dest_copia[i2].x;
    dest_gpu[i2].y = U_gpu[i2]*dest_copia[i2].y;
}	
...
Fresnel(U_gpu, dest_copia, dest_gpu, m_gpu, n_gpu, np.float32(dx), np.float32(dy), np.float32(
